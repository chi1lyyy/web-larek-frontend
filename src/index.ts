...
function cloneTemplate<T extends HTMLElement>(query: string | HTMLTemplateElement): T {
    const template = query as HTMLTemplateElement;
    return template.content.firstElementChild.cloneNode(true) as T;
}
cardsData.forEach(card => {
    const template = document.getElementById('card-catalog') as HTMLTemplateElement;
    const cardClone = cloneTemplate(template);
    ...
    cardClone.querySelector('.card__price').textContent = `${card.price} синапсов`;
